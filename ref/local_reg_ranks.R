# see:https://cran.r-project.org/web/packages/KernSmooth/KernSmooth.pdf
...
data(prca)
prca
# sort data by age and then weight
prca %>%
  arrange(age, weight) -> prca
cox.result = coxph(Surv(survtime,cens) ~ rx, data = prca)
summary(cox.result)
prca$weight. = (prca$weight)
prca$age. = (prca$age)
prca$weight = rank(prca$weight)
prca$age = rank(prca$age)
prca$age
unit. = 1
...
grid.pts.x = sort(unique(prca$age))
grid.pts.y = sort(unique(prca$weight))
# 
# grid.pts.x = seq(x.lim[1], x.lim[2], unit.x)
# grid.pts.y = seq(y.lim[1], y.lim[2], unit.y)
grid.xy = expand.grid(grid.pts.x, grid.pts.y)
grid.xy = expand.grid(sort(grid.pts.x), sort(grid.pts.y))
names(grid.xy) = c("age", "weight")
i=1
n.cutoff = 20


aaa = data.frame(prca$age, prca$weight)
# i=5000
# Now do a bivariate local regression ####
nrow(grid.xy)
# results.xy = do.call(rbind, lapply(1:nrow(grid.xy), FUN = function(i){
...
results.xy = do.call(rbind, lapply(1:nrow(grid.xy), FUN = function(i){
  ...
  rank_weight = exp(-mahalanobis(x = data.frame(prca$age, prca$weight),
                             center = c(center.x, center.y),
                             cov    = matrix(c(var,0,0,var), nrow = 2))/2)
  weight.all = exp(-mahalanobis(x = grid.xy,
                             center = c(center.x, center.y),
                             cov    = matrix(c(var,0,0,var), nrow = 2))/2)
      # # Check plot
      #  filled.contour(sort(grid.pts.x), sort(grid.pts.y),
      #                 matrix(weight.all, nrow = length(grid.pts.x)))
      ...
      # points(prca$age[rank_weight>weight.up],
      #        prca$weight[rank_weight>weight.up],
      #        col = "red")
      # contour(sort(grid.pts.x), sort(grid.pts.y),
      #         matrix(weight.all, nrow = length(grid.pts.x)),
      #         col = "red", add = TRUE)
      # circle1 <- circleFun(c(center.x, center.y), x.conf.up - center.x, npoints = 100)
      # lines(circle1$x, circle1$y, col = "green", lwd = 2)
      # # Check plot END
  # cbind(prca$age, prca$weight, round(weight.,5), prca$age.. prca$weight.) -> aaa
  # hist(weight.)
  # sum(weight.>weight.up)>n.cutoff
...
par(mar=c(5,4,4,2))
# plot(prca$age., prca$weight., type = "n",
#      yaxs="i",xaxs="i",
#      # xlim = range(grid.pts.x),
#      # ylim = range(grid.pts.y),
...
ref.x = data.frame(age = prca$age, age. = prca$age.)
ref.y = data.frame(weight = prca$weight, weight. = prca$weight.)
...
results.xy %>%
  ...
  left_join(ref.x) %>%
  left_join(ref.y) %>% 
  distinct(age, weight, .keep_all = T) -> plot.dat2
head(plot.dat2)
z.matrix = matrix(plot.dat2$estimate,
                  nrow = length(grid.pts.x),
                  ncol = length(grid.pts.y), byrow = F)
rownames(z.matrix) = grid.pts.x
colnames(z.matrix) = grid.pts.y
...
head(results.xy)
# filled.contour(grid.pts.x, grid.pts.y, z.matrix, zlim = c(-3,3))
# filled.contour(unique(sort(ref.x$age.)), unique(sort(ref.y$weight.)), z.matrix)
...
plot.dat$estimate
points(plot.dat$age., plot.dat$weight., cex = 0.5)
