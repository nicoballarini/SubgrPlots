...
#' @export
barcht <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type, font.size = c(15, 12, 10, 0.6), title = NULL, lab.y = NULL)
{
  ...
  pushViewport(vp)
  #grid.text("Treatment effect sizes across subgroups", gp = gpar(fontsize = font.size[1], fontface = 2))
  ...
  breaks <- seq(min(c(treatment.mean), na.rm = T) - 1e-8, max(c(treatment.mean), na.rm = T) + 1e-8, length.out = 100)
  ...
  grid.yaxis(seq(0,1,  by=0.1), vp = viewport(y=0.5), label = round(seq(axis.min, axis.max, len = 11),2),
             gp = gpar(cex = font.size[4]), edits = gEdit(gPath ="labels", rot=90))
  ...
  grid.text(lab.y, gp = gpar(fontsize = font.size[2], fontface = 2), rot = 90)                  # place the label of the y-axis
