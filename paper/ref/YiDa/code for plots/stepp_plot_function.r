...
#' @export
stepp.plt <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type, setup.ss, alpha, font.size = c(1.2,1,1,0.85), title = NULL, lab.y = NULL,
                      subtitle = NULL)
{
  ...
  plot(c(1 : n.subgrp.covar1), treatment.mean, type = "o", lwd = 1.5, pch = 16, lty = 1,  col = "red", xaxt = "n",
       ylim = c(min(treatment.lower, na.rm = TRUE), max(treatment.upper, na.rm = TRUE)),
       xlim = c(1, n.subgrp.covar1),
       xlab = lab.var, ylab = lab.y, main = title,
       sub = subtitle, cex.main = font.size[1],
       cex.lab = font.size[2], cex.sub =  font.size[3])
