...
#' @export
labbeplt <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type, size.shape = 1/18, adj.ann.subgrp = 1/30, font.size = c(1, 1, 0.85, 0.85),
                     title = NULL, lab.xy = NULL)
{
  ...
  plot(treatment.C.mean, treatment.T.mean, type='n',  xlab = lab.xy[[1]], ylab = lab.xy[[2]], main= title,
       ylim = c(x.lim.min, x.lim.max), xlim = c(x.lim.min, x.lim.max), cex.main = font.size[1], cex.lab = font.size[2], bty = "o")
  abline(a=0, b = 1, lty = 2)
  text( max(max(treatment.T.mean, na.rm = TRUE),  max(treatment.C.mean, na.rm = TRUE)),  max(max(treatment.T.mean, na.rm = TRUE),
                                                                                             max(treatment.C.mean, na.rm = TRUE)), labels = "x = y")
  ...
  col.seg = rep("blue", n.subgrp.tol + 1)
  col.seg[which(treatment.mean < 0 )] = rep("red", length(which(treatment.mean < 0 )))
  ...
  for (i in 1: (n.subgrp.tol + 1)){
    ...
    segments(treatment.C.mean[i], treatment.C.mean[i], treatment.C.mean[i], CI.bar.y.pos[i], col = "violet", lwd = 1, lty = 2)
  }
  ...
  text(x.pos + lab.pos.adj * 8, y.pos, labels = round(treatment.mean[1 : (n.subgrp.tol + 1)], 2), col = col.seg[1 : (n.subgrp.tol + 1)],
       adj = c(0, 1),  cex = font.size[3])

  text(x.pos[n.subgrp.tol] + lab.pos.adj * 8, y.pos[1] + lab.pos.adj, labels = "Effect size", adj = c(0, 1), cex = font.size[3], font=4)
