...
#' @export
forestplt <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type,
                      size.shape = c(0.25, 0.12), font.size = c(1.3, 1, 0.85, 0.9),
                      title = NULL, lab.x = NULL)
{
    ...
    if (sum((data.subgrp[[i]]$trt == "1")) == 0 | sum((data.subgrp[[i]]$trt == "0")) == 0){
      ...
      if (outcome.type == "continuous"){
        ...
        if (length(which(data.subgrp[[i]]$trt == 0)) == 0){
          ...
          model.sum = summary(model.int)
          treatment.C.mean[i] = NA # model.sum$coefficients[1, 1]
          treatment.C.upper[i] = NA # model.sum$coefficients[1, 1] + 1.96 * model.sum$coefficients[1, 2]
          treatment.C.lower[i] = NA # model.sum$coefficients[1, 1] - 1.96 * model.sum$coefficients[1, 2]
        }
        ...
        if (length(which(data.subgrp[[i]]$trt == 1)) == 0){
          ...
          model.sum = summary(model.int)
          treatment.T.mean[i] = NA # model.sum$coefficients[1, 1]
          treatment.T.upper[i] = NA # model.sum$coefficients[1, 1] + 1.96 * model.sum$coefficients[1, 2]
          treatment.T.lower[i] = NA # model.sum$coefficients[1, 1] - 1.96 * model.sum$coefficients[1, 2]
        }
      }
    }
  }
  ...
  if (sum((dat$trt == "1")) == 0 | sum((dat$trt == "0")) == 0){
    ...
    if (outcome.type == "continuous"){
      ...
      if (length(which(dat$trt == 0)) == 0){
        ...
        model.sum = summary(model.int)
        treatment.C.mean[n.subgrp.tol + 1] = NA #model.sum$coefficients[1, 1]
        treatment.C.upper[n.subgrp.tol + 1] = NA #model.sum$coefficients[1, 1] + 1.96 * model.sum$coefficients[1, 2]
        treatment.C.lower[n.subgrp.tol + 1] = NA #model.sum$coefficients[1, 1] - 1.96 * model.sum$coefficients[1, 2]
      }
      ...
      if (length(which(dat$trt == 1)) == 0){
        ...
        model.sum = summary(model.int)
        treatment.T.mean[n.subgrp.tol + 1] = NA # model.sum$coefficients[1, 1]
        treatment.T.upper[n.subgrp.tol + 1] = NA # model.sum$coefficients[1, 1] + 1.96 * model.sum$coefficients[1, 2]
        treatment.T.lower[n.subgrp.tol + 1] = NA # model.sum$coefficients[1, 1] - 1.96 * model.sum$coefficients[1, 2]
      }
    }
  }
  ...
  if (outcome.type == "survival"){
    layout(matrix(c(1,2), byrow = TRUE, nrow=1, ncol=2), widths=c(1, 1), heights=c(1, 1))
  }else{
  ...
  plot(0, 0, type='n',  xlab = lab.x[[1]], ylab = "",
       ylim = c(0, y.lim.max), xlim = c(-x.lim.max - 4, -x.lim.max + 0.6), xaxt="n", yaxt="n", main = title[[1]])
  ...
  for (i in 1: (n.subgrp.tol + 1)){
    text(-x.lim.max-3.2, line.y.range[i,1], labels = lab.subgrp[i], cex = font.size[3])
    text(-x.lim.max-1.8, line.y.range[i,1], labels = round(treatment.mean[i],2), cex = font.size[3])
    text(-x.lim.max-1.2, line.y.range[i,1], labels = round(treatment.lower[i],2), cex = font.size[3])
    text(-x.lim.max-0.6, line.y.range[i,1], labels = round(treatment.upper[i],2), cex = font.size[3])
    text(-x.lim.max + 0.2, line.y.range[i,1], labels = ss.subgrp.list[i], cex = font.size[3])
  }

  sub.title.xpos = c(-x.lim.max-1.8, -x.lim.max-1.2, -x.lim.max-0.6, -x.lim.max + 0.2)
  sub.title.ypos = rep(line.y.range[n.subgrp.tol + 1, 1] + 0.1, 4)

  sub.title = c("Mean", "Low.", "Upp.", "S.Size(T|C)")
  text(sub.title.xpos, sub.title.ypos, labels = sub.title, cex = font.size[3], font=4)
  ...
  x.lim.min = round(est.range[dim(est.range)[1],1],2) - 3
  x.lim.min = round(x.lim.min/0.5) * 0.5
  x.lim.max = x.lim.min + 6
  ...
  for (i in 1: (n.subgrp.tol)){
    lines(line.x.range[i, ], line.y.range[i, ], col = "royalblue")
    ...
    polygon(x, y,col = "royalblue", border = "royalblue")
  }
  lines(line.x.range[n.subgrp.tol + 1, ], line.y.range[n.subgrp.tol + 1, ], col = "purple1")
  x0 = line.centre[n.subgrp.tol + 1]; y0 = line.y.range[n.subgrp.tol + 1, 1]
  x = c(x0 - w, x0, x0 + w, x0, x0 - w)
  y = c(y0, y0 - h, y0, y0 + h, y0)
  polygon(x, y,col = "purple1", border = "purple1")
  ...
  if (outcome.type != "survival"){
    ...
    for (i in 1: (n.subgrp.tol)){
      lines(line.C.x.range[i, ], line.C.y.range[i, ], col = "royalblue")
      ...
      lines(line.T.x.range[i, ], line.T.y.range[i, ], col = "royalblue")
    ...
    lines(line.C.x.range[n.subgrp.tol + 1, ], line.C.y.range[n.subgrp.tol + 1, ], col = "purple1")
    ...
    lines(line.T.x.range[n.subgrp.tol + 1, ], line.T.y.range[n.subgrp.tol + 1, ], col = "purple1")
