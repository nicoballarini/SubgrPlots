...
#' @export
radplt <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type, range.v = NULL, adj.ann.subgrp = 4, font.size = c(1, 1, 0.85, 0.85, 1),
          title = NULL, lab.xy = NULL)
{
  ...
  for (i in 1 : (n.subgrp.tol)){
    if (sum((data.subgrp[[i]]$trt == "1")) == 0 | sum((data.subgrp[[i]]$trt == "0")) == 0){
      ...
      if (outcome.type == "continuous"){
        ...
        zscore[i] = treatment.mean[i] / treatment.std.full                              # calculate the z-score of the subgroup effect size relative
        ...
        zscore[i] = treatment.mean[i] / treatment.std.full
      }
    }
  ...
  plot(0, 0, type='n',  xlab = lab.xy[[1]], ylab = lab.xy[[2]], ylim = c(y.lim.min, y.lim.max),
       xlim = c(0,13.5), xaxt = 'n', yaxt='n', main = title, cex.lab = font.size[2], cex.main = font.size[1])
  ...
  x.lab.max.pos = round(max(1/treatment.std)) + 0.1 #ceiling(max(1/treatment.std))
  ...
  lab = c(rep(NA, len = length(seq(y.lim.min, lower.bd - 1, 1))), c(lower.bd, NA, 0, NA, upper.bd),
          rep(NA, len = length(seq(upper.bd +1, y.lim.max, 1))))
  axis(2, at= seq(y.lim.min + zscore.full, y.lim.max + zscore.full, 1), labels = lab, cex.axis = font.size[5] )

  #axis(2, at= seq(y.lab.low.pos - 1, y.lab.upp.pos + 1, 1),
  #     labels = c(NA, lower.bd, NA, 0, NA, upper.bd, NA), cex.axis = font.size[5] )
  ...
  points(1/treatment.std * expand.fac.adj, zscore, cex = 0.8, pch = 19)             # add the points for the subgroups
  text((1/treatment.std) * expand.fac.adj + lab.subgrp.pos.adj  * expand.fac.adj,
       zscore + lab.subgrp.pos.adj, labels = lab.subgrp2,  cex = font.size[4])      # the annotation for the points
