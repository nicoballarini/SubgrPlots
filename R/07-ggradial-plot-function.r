...
#' @import grid
#' @import graphics
ggplot_radial <- function(dat, covari.sel, trt.sel, resp.sel, outcome.type, range.v = NULL,
  ...
  if (sum(angle>0) != length(angle)){                                               # when the zscores of subgroups are partially larger and less than the zscore of the full population
    ...
    for (i in 1: length(angle.neg)){
      x1 = x0 + seq(r * cos(angle.neg[i]), r1 * cos(angle.neg[i]), len=10)
      y1 = y0 + seq(r * sin(angle.neg[i]), r1 * sin(angle.neg[i]), len=10)
      lines(x1, y1, col = "blue")# drew the lines for the unit used in effect size
    }
  ...
  library(ggrepel)
  dat_plot = data.frame(
    "x" = 1/c(treatment.std) * expand.fac.adj,
    "y" = c(zscore),
    label = lab.subgrp2,
    subgroup_label = rownames(treatment.std)
  )
  y_breaks = c(-2,0,2)
  ggplot() +
    geom_rect(aes(xmin = c(0),
                  xmax = c(xlim[2]),
                  ymin = c(-2),
                  ymax = c(2)), fill = "gray80") +
    geom_hline(yintercept = c(-2,2), linetype = 2) +
    geom_hline(yintercept = c(0), linetype = 1) +
    geom_point(aes_string(x = "x", y = "y", color = "subgroup_label"), data = dat_plot) +
    geom_point(aes_string(x = "x", y = "y"), data = dat_plot) +
    geom_text_repel(aes_string(x = "x", y = "y", label = "label"), data = dat_plot) +
    geom_line(aes_string(x = "x", y = "y"), data = data_angle1) +
    geom_line(aes_string(x = "x", y = "y"), data = data_angle2) +
    scale_x_continuous(name = lab.xy[[1]], limits = xlim) +
    scale_y_continuous(name = lab.xy[[2]], limits = ylim,
                       breaks = y_breaks) +
    scale_color_manual(name = NULL, values = rep("white", nrow(dat_plot))) +
    coord_cartesian(expand = FALSE) +
    theme_bw() +
    theme(panel.grid = element_blank(),
          legend.key.width = unit(0, units = "cm"),
