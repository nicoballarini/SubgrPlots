...
#   }
  ...
  Set_size_data$total_size = total_size
  Size_plot <- (ggplot(data = Set_size_data, aes_string(x ="x", y = "y"))
                + geom_bar(aes_string(x ="x", y = "total_size"),
                           stat = "identity", colour = sbar_color, width = 0.4,
                           fill = "white",
                           position = "identity")
...
Make_size_plot_t <- function(Set_size_data, sbar_color, ratios, ylabel, scale_sets, text_scale, set_size_angle, total_size){
  #   if(ratios[1] < 0.4){
  #     m <- (-0.05)
  #   }
  #   else if((ratios[1] > 0.4) & (ratios[1] < 0.46)){
