...
#                                                                                                                                                      #
# alt.unidir.aw.ovlp:  this function produces a plot for displaying relative proportions of pairwise subgroup overlap, where the relative proportion   #
...
# para:         a vector with three elements specifying the parameters of plot display; the first element is for specifying the length of the arrowhead;
...
#' @export
alt.unidir.aw.ovlp <- function(dat, covari.sel, para = c(0.5, 0.15, 1), mode, font.size, title = NULL)
{
  ...
  if (mode == 1){
    ...
    for (i in 1:n.subgrp.pair){
      Arrows(x1[st[i]], y1[st[i]], x1[se[i]], y1[se[i]], arr.length = para[1], arr.width = para[2], arr.adj = para[3],
             code = 2, arr.type = "curved",  arr.col = col.idx[i], lcol = col.idx[i], lwd = 2)
      Arrows(x2[se[i]], y2[se[i]], x2[st[i]], y2[st[i]], arr.length = para[1], arr.width = para[2], arr.adj = para[3],
             code = 2, arr.type = "curved",  arr.col = col.idx.rev[i], lcol = col.idx.rev[i], lwd = 2)
    }
