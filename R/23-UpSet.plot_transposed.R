...
Make_base_plot_t <- function(Main_bar_plot, Matrix_plot, Size_plot, labels, hratios, att_x, att_y,
  ...
  # grid.draw(arrangeGrob(Size_plot, Matrix_plot, heights = c(0.3,0.7)))
...
BaseBoxPlot_t <- function(box_plot, position, size_plot_height, Main_bar_plot, Matrix_plot,
  ...
  # grid.draw(arrangeGrob(Matrix_plot))
  # grid.draw(arrangeGrob(Size_plot, Matrix_plot, heights = c(0.3,0.7)))
  
  grid.draw(arrangeGrob(Matrix_plot, box_plot[[1]], Main_bar_plot, widths = hratios))
    # grid.draw(arrangeGrob(grid::grid.rect(gp = gpar(fill=NA)),
    #                     grid::grid.rect(gp = gpar(fill=NA)),
    #                     grid::grid.rect(gp = gpar(fill=NA)),
    #                     widths = hratios))
    popViewport()
  # vp = vplayout(bar_top:(size_plot_height-2), matrix_and_mainbar_left:matrix_and_mainbar_right, angle=90)
  # vp = viewport(x = 0.5, y = 0.5, width = 0.7, height = 0.5, angle=-90)
  # pushViewport(vp)
  # grid::grid.rect()
  # grid.draw(Main_bar_plot)
  # popViewport()
  # # vp = vplayout(size_plot_height:matrix_bottom, matrix_and_mainbar_left:matrix_and_mainbar_right)
  # vp = viewport(x = 0.15, y = 0.5, width = 0.7, height = 0.25, angle=-90)
  # pushViewport(vp)
  ...
  # vp = vplayout(matrix_and_mainbar_left:matrix_and_mainbar_right, (att_top):att_bottom)
  # pushViewport(vp)
  # # grid::grid.rect()
  # grid.draw(arrangeGrob(box_plot[[1]]))
  # popViewport()
  # if(length(box_plot) == 2){
  #   vp = vplayout(matrix_and_mainbar_left:matrix_and_mainbar_right,(att_bottom + 10):(att_bottom + 25))
  #   pushViewport(vp)
...
NoAttBasePlot <- function(legend, size_plot_height, Main_bar_plot, Matrix_plot, hratios,
  ...
  # popViewport()
  vp = vplayout(top:(size_plot_height-1), matrix_and_mainbar_left:matrix_and_mainbar_right)
  pushViewport(vp)
  grid.draw(Main_bar_plot)
  popViewport()
  vp = vplayout(size_plot_height:bottom, matrix_and_mainbar_left:matrix_and_mainbar_right)
  pushViewport(vp)
  grid.draw(Matrix_plot)
  popViewport()
