...
#                                                                                                                                                   #
#  mat.subgrp.op:  this function produces a matrix plot for displaying relative proportions of pairwise subgroup overlap, where the relative propo- #
#                  -rtion is defined by |intersect(A, B)|/|A|, for any sets A and B and A is the baseline set. The letters over (and beside) the ma-#
#                  -trix represent subgroups defined by the categories of the selected covariates. The subgroup letters on the rows are regarded as #
#                  baseline subgroups for calculating  relative overlap proportions. There are two modes of display - one with circles which are    #
...
#' @export
mat.subgrp.op <- function(dat, covari.sel, mode, font.size = c(2, 1), title = NULL)
{
  ...
  layout(matrix(c(1,1, 1,1, 1, 1, 2, 2), byrow = TRUE, nrow=4, ncol=2), heights=c(5.5,1))
  par(mar=c(0,2,2,2))
  ...
  text(rep(0.25, n.subgrp.tol), seq(10.5 - cell.height/2, 0.5 + cell.height/2, len = n.subgrp.tol),
                                                           labels= lab.subgrp, cex = font.size[2])      # place subgroup labels vertically
  text(seq(0.5 + cell.width/2, 10.5 - cell.width/2, len = n.subgrp.tol), rep(10.75, n.subgrp.tol),
  ...
  if (mode == 1){
    ...
    for (i in 1 :(n.subgrp.tol)){
      ...
      rect(xl, yb, xr, yt,   border = "black", col="yellow")
    }
  }
